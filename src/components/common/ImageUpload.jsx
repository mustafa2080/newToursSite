...
const ImageUpload = ({
  ...
  // Use advanced compression utility
  const convertToBase64 = async (file) => {
    try {
      const settings = getOptimalSettings(file, compressionType);
  ...
  const uploadFiles = async (files) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // Convert to base64 for reliable storage (avoiding Firebase Storage CORS issues)
        const compressionResult = await convertToBase64(file);

        const imageData = {
          id: fileId,
          url: compressionResult.base64, // Use compressed base64
          name: file.name,
          size: file.size,
          ...
          type: file.type,
          file: file, // Keep file reference for backup
        };
