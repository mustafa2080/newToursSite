...
const TripsManagement = () => {
  ...
  // Helper function to get image URL from trip data
  const getImageUrl = (trip) => {
    // Check for compressed image format first
    if (trip.mainImage && typeof trip.mainImage === 'object' && trip.mainImage.base64) {
      return trip.mainImage.base64;
    }

    // Check for legacy image fields
    const legacyImage = trip.mainImage || trip.main_image || trip.image || trip.photo;
    if (legacyImage && typeof legacyImage === 'string') {
      return legacyImage;
    }

    // Fallback to placeholder
    return `https://picsum.photos/200/120?random=${trip.id || Math.random()}`;
  };
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-shrink-0 relative">
                    <img
                      ...
                      onError={(e) => {
                        e.target.src = `https://picsum.photos/200/120?random=${trip.id || Math.random()}`;
                      }}
                    />
                    {trip.mainImage && typeof trip.mainImage === 'object' && trip.mainImage.compressed && (
                      ...
                      <div className="flex items-center space-x-6 text-sm text-gray-600">
                        ...
                        {trip.mainImage && typeof trip.mainImage === 'object' && trip.mainImage.compressed && (
                          <div className="flex items-center text-green-600">
                            ...
                            {trip.mainImage.compressionRatio}% compressed
                          </div>
                        )}
                      </div>
