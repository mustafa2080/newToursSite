...
export const getTrips = async (filters = {}) => {
  try {
    ...
    // Filter for active trips (include trips without status)
    trips = trips.filter(trip => trip.status === 'active' || !trip.status);
...
export const getFeaturedTrips = async (limitCount = 6) => {
  try {
    ...
    // Client-side filtering - show featured trips first, then all active trips
    let featuredTrips = trips.filter(trip =>
      trip.featured === true && (trip.status === 'active' || !trip.status)
    );

    // If no featured trips found, show all active trips
    if (featuredTrips.length === 0) {
      featuredTrips = trips.filter(trip =>
